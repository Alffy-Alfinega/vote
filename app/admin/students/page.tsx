...
export default function StudentsPage() {
  ...
  const [editForm, setEditForm]       = useState({ name:"", class_name:"", pin:"" });
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(`${data.name} added successfully.`); load(); setTimeout(() => setSuccess(""), 4000);
    } catch (err: unknown) { setError(err instanceof Error ? err.message : "Error"); }
    finally { setSaving(false); }
  }

  function openEdit(s: Student) {
    setEditStudent(s); setEditForm({ name:s.name, class_name:s.class_name||"", pin:"" }); setEditError("");
  }
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed");
      setEditStudent(null); setSuccess(`${data.name} updated.`); load(); setTimeout(() => setSuccess(""), 4000);
  ...
  return (
    <div style={{ padding:"1rem", maxWidth:"72rem", margin:"0 auto" }} className="sm:p-6 lg:p-8">
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"1rem", marginBottom:"1.5rem", flexWrap:"wrap" }}>
        ...
        <div style={{ display:"flex", gap:".5rem", flexWrap:"wrap" }}>
          <button onClick={() => { setShowImport(!showImport); setShowForm(false); }} className="btn-ghost"
            style={{ border:"1px solid var(--color-ash-200)", fontSize:".875rem" }}>↑ Import CSV</button>
          <button onClick={() => { setShowForm(!showForm); setShowImport(false); }} className="btn-primary" style={{ fontSize:".875rem" }}>+ Add Student</button>
        </div>
      </div>
      ...
      {showImport && (
        <div className="card" style={{ padding:"1.25rem", marginBottom:"1.25rem", border:"1px solid color-mix(in srgb,var(--color-gold-500) 30%,transparent)" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1rem", flexWrap:"wrap", gap:".5rem" }}>
            ...
            <button onClick={downloadTemplate} className="btn-ghost" style={{ border:"1px solid var(--color-ash-200)", fontSize:".8125rem", padding:".5rem .875rem", minHeight:40 }}>↓ Template</button>
          </div>
          ...
          {importResult && (
            <p style={{ fontSize:".875rem", marginTop:".875rem" }}>
        ...
        <div className="card" style={{ padding:"1.25rem", marginBottom:"1.25rem", border:"1px solid color-mix(in srgb,var(--color-gold-500) 30%,transparent)" }}>
          ...
          <form onSubmit={handleAdd} style={{ display:"flex", flexDirection:"column", gap:".875rem" }}>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:".875rem" }}>
              <div><label style={{ display:"block", fontSize:".8125rem", fontWeight:500, marginBottom:".375rem" }}>Student ID *</label>
                <input className="input" placeholder="e.g. S2024001" value={form.student_id} onChange={setF("student_id")} required/></div>
              <div><label style={{ display:"block", fontSize:".8125rem", fontWeight:500, marginBottom:".375rem" }}>Full Name *</label>
                <input className="input" placeholder="e.g. Aisha Nakato" value={form.name} onChange={setF("name")} required/></div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:".875rem" }}>
              <div><label style={{ display:"block", fontSize:".8125rem", fontWeight:500, marginBottom:".375rem" }}>Class</label>
                <input className="input" placeholder="e.g. S.4 East" value={form.class_name} onChange={setF("class_name")}/></div>
              <div><label style={{ display:"block", fontSize:".8125rem", fontWeight:500, marginBottom:".375rem" }}>PIN *</label>
                <input type="password" className="input" placeholder="4–8 digit PIN" maxLength={8} value={form.pin} onChange={setF("pin")} inputMode="numeric" required/></div>
            </div>
      ...
      <div style={{ position:"relative", marginBottom:"1rem" }}>
        <svg style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", width:16, height:16, color:"var(--color-ash-300)" }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
        </svg>
        <input className="input" style={{ paddingLeft:"2.75rem" }} placeholder="Search by name, ID, or class…" value={search} onChange={e => setSearch(e.target.value)}/>
      </div>
          ...
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead style={{ background:"var(--color-ash-50)", borderBottom:"1px solid var(--color-ash-100)" }}>
                <tr>{["Student","ID","Class","Registered","Actions"].map(h => <th key={h} className="table-cell table-head" style={{ textAlign:"left" }}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {filtered.map(s => (
      ...
      {editStudent && (
        <div style={{ position:"fixed", inset:0, zIndex:50, display:"flex", alignItems:"center", justifyContent:"center", padding:"1rem" }}>
          <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.5)" }} onClick={() => setEditStudent(null)}/>
          <div className="card" style={{ position:"relative", width:"100%", maxWidth:480, padding:"1.5rem", zIndex:1 }}>
            <h2 style={{ fontFamily:"var(--font-display)", fontWeight:600, fontSize:"1.25rem", marginBottom:"1.25rem" }}>Edit Student</h2>
            <form onSubmit={handleEdit} style={{ display:"flex", flexDirection:"column", gap:".875rem" }}>
              <div>
                <label style={{ display:"block", fontSize:".8125rem", fontWeight:500, marginBottom:".375rem" }}>Student ID</label>
                <input className="input" value={editStudent.student_id} disabled style={{ opacity:.6, cursor:"not-allowed" }}/>
                <p style={{ fontSize:".75rem", color:"var(--color-ash-300)", marginTop:".25rem" }}>Student ID cannot be changed.</p>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:".875rem" }}>
                <div><label style={{ display:"block", fontSize:".8125rem", fontWeight:500, marginBottom:".375rem" }}>Full Name *</label>
                  <input className="input" value={editForm.name} onChange={e => setEditForm(p => ({...p, name:e.target.value}))} required/></div>
                <div><label style={{ display:"block", fontSize:".8125rem", fontWeight:500, marginBottom:".375rem" }}>Class</label>
                  <input className="input" value={editForm.class_name} onChange={e => setEditForm(p => ({...p, class_name:e.target.value}))}/></div>
              </div>
              <div>
                <label style={{ display:"block", fontSize:".8125rem", fontWeight:500, marginBottom:".375rem" }}>New PIN <span style={{ color:"var(--color-ash-300)", fontWeight:400 }}>(leave blank to keep current)</span></label>
                <input type="password" className="input" placeholder="Leave blank to keep current PIN" maxLength={8} inputMode="numeric"
                  value={editForm.pin} onChange={e => setEditForm(p => ({...p, pin:e.target.value}))}/>
              </div>
              {editError && <div style={{ background:"#fef2f2", border:"1px solid #fecaca", borderRadius:".75rem", padding:".75rem 1rem" }}><p style={{ fontSize:".875rem", color:"#dc2626" }}>{editError}</p></div>}
              <div style={{ display:"flex", gap:".75rem", paddingTop:".25rem" }}>
                ...
                <button type="button" className="btn-ghost" style={{ border:"1px solid var(--color-ash-200)" }} onClick={() => setEditStudent(null)}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
      )}
      <style>{`.animate-spin{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.opacity-25{opacity:.25}.opacity-75{opacity:.75}`}</style>
    </div>
  );
}
